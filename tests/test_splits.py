...
def _make_df(n_snippets: int, rows_per_snippet: int = 6) -> pl.DataFrame:
    rows = [
        {"code_snippet_id": cid, "value": f"row-{cid}-{i}"}
...
def test_split_is_deterministic():
    df = _make_df(100)
    train1, test1 = split_by_key(df, "code_snippet_id", 0.30, seed=0)
    train2, test2 = split_by_key(df, "code_snippet_id", 0.30, seed=0)
...
def test_split_different_seeds_differ():
    df = _make_df(100)
    train1, _ = split_by_key(df, "code_snippet_id", 0.30, seed=0)
    train2, _ = split_by_key(df, "code_snippet_id", 0.30, seed=42)
    assert not train1.equals(train2)


def test_split_keys_are_disjoint():
    df = _make_df(100)
    train, test = split_by_key(df, "code_snippet_id", 0.30, seed=0)
    train_ids = set(train["code_snippet_id"].to_list())
    test_ids = set(test["code_snippet_id"].to_list())
...
def test_split_ratio_is_approximate():
    df = _make_df(200)
    _, test = split_by_key(df, "code_snippet_id", 0.30, seed=0)
    unique_test = test["code_snippet_id"].n_unique()
...
def test_split_both_sides_non_empty(test_frac: float):
    df = _make_df(20)
    train, test = split_by_key(df, "code_snippet_id", test_frac, seed=0)
